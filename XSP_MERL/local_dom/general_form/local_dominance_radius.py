...
This file intentionally does NOT import general_model.py.  It assumes the first
workflow has already been run and that the following files already exist inside
an instance folder:
    ...
    def solve_by_dual_extreme_enumeration(
        ...
        result = {
            ...
            "notes": [
                ...
                "The primary local-dominance model is built by build_pyomo_model() and solved by solve_pyomo().",
            ],
        }
...
def run_local_dominance_radius(
    ...
    problem = LocalDominanceRadiusProblem(instance_name, base_dir=base_dir, scenario=scenario, gap_tol=gap_tol)
    if method == "pyomo":
        return problem.solve_pyomo(solver=solver, tee=tee)
    if method == "enumeration":
        return problem.solve_by_dual_extreme_enumeration()
    raise ValueError("method must be 'pyomo' or 'enumeration'.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--method", choices=["pyomo", "enumeration"], default="pyomo", help="Default uses Pyomo/Gurobi.")
