...
This script solves:
    ...
    4. The scenario-wise regret decomposition:

           Delta_s = Q_s(x_EV) - Q_s(x_star)
           Delta_0 = c*x_EV - c*x_star

       together with good/bad scenario partitions, compensation budget,
       VSS decomposition, and a minimum-cardinality subset of bad scenarios
       whose weighted regrets overcome the compensation term.

Outputs:
    - Printed terminal summary.
    - A text summary file.
    - CSV files with scenario data, model results, recourse evaluations,
      comparison metrics, regret decomposition, good/bad scenario sets,
      and the minimal bad scenario subset.

Run:
    python dummy_demand_TSSP_with_regret.py

Optional:
    python dummy_demand_TSSP_with_regret.py --output-dir my_results
    python dummy_demand_TSSP_with_regret.py --solver gurobi --tee
"""
...
# -----------------------------------------------------------------------------
# Regret decomposition and minimal bad-scenario subset
# -----------------------------------------------------------------------------
...
def compute_regret_decomposition(
    ...
    """
    Compute the scenario-wise regret decomposition.

    Delta_s = Q_s(x_EV) - Q_s(x_star)
    Delta_0 = c*x_EV - c*x_star

    Good scenarios: Delta_s <= 0
    Bad scenarios:  Delta_s > 0

    C_comp = Delta_0 + sum_{s in G} p_s Delta_s

    VSS = sum_{s in B} p_s Delta_s + C_comp

    The minimum bad subset is the smallest-cardinality subset K of bad scenarios
    such that:

        sum_{s in K} p_s Delta_s + C_comp > 0.

    Because each p_s Delta_s is positive for bad scenarios, sorting bad scenarios
    by descending p_s Delta_s and taking the largest weights until the inequality
    is satisfied gives a minimum-cardinality subset.
    """
    stoch_by_s = {int(row["scenario"]): row for row in stoch_eval_rows}
    ev_by_s = {int(row["scenario"]): row for row in ev_eval_rows}

    delta_0 = ev_eval_summary["first_stage_cost"] - stoch_eval_summary["first_stage_cost"]
    ...
    for s in data.scenarios:
        q_star = stoch_by_s[s]["Q_s_x"]
        q_ev = ev_by_s[s]["Q_s_x"]
        delta_s = q_ev - q_star
        weighted_delta_s = data.probability[s] * delta_s
        classification = "good" if delta_s <= EPS else "bad"
    ...
    good_weighted_sum = sum(row["p_s_Delta_s"] for row in good_rows)
    bad_weighted_sum = sum(row["p_s_Delta_s"] for row in bad_rows)
    compensation_budget = delta_0 + good_weighted_sum
    vss_from_decomposition = bad_weighted_sum + compensation_budget
    decomposition_error = vss_from_decomposition - vss

    sorted_bad_rows = sorted(
        bad_rows,
        key=lambda row: (row["p_s_Delta_s"], row["Delta_s"]),
        reverse=True,
    )

    minimal_subset_rows: List[Dict[str, Any]] = []
    running_weighted_delta = 0.0
    selected_total_with_comp = compensation_budget

    if vss <= EPS:
        minimal_subset_status = "not_computed_because_VSS_is_not_positive"
    elif compensation_budget > EPS:
        minimal_subset_status = "empty_subset_satisfies_constraint"
    else:
        minimal_subset_status = "not_found"
        for rank, row in enumerate(sorted_bad_rows, start=1):
            running_weighted_delta += row["p_s_Delta_s"]
            selected_total_with_comp = compensation_budget + running_weighted_delta

            selected_row = dict(row)
            selected_row["selection_rank"] = rank
            selected_row["cumulative_p_s_Delta_s"] = running_weighted_delta
            selected_row["cumulative_plus_C_comp"] = selected_total_with_comp
            minimal_subset_rows.append(selected_row)

            if selected_total_with_comp > EPS:
                minimal_subset_status = "found"
                break

    if minimal_subset_status == "empty_subset_satisfies_constraint":
        running_weighted_delta = 0.0
        selected_total_with_comp = compensation_budget
    ...
    regret_summary_rows = [
        {
            "metric": "Delta_0",
            "value": delta_0,
            "description": "First-stage difference c*x_EV - c*x_star",
        },
        {
            "metric": "good_scenario_weighted_delta_sum",
            "value": good_weighted_sum,
            "description": "sum_{s in G} p_s Delta_s",
        },
        {
            "metric": "bad_scenario_weighted_delta_sum",
            "value": bad_weighted_sum,
            "description": "sum_{s in B} p_s Delta_s",
        },
        {
            "metric": "C_comp",
            "value": compensation_budget,
            "description": "Delta_0 + sum_{s in G} p_s Delta_s",
        },
        {
            "metric": "VSS",
            "value": vss,
        ...
        {
            "metric": "VSS_from_decomposition",
            "value": vss_from_decomposition,
            "description": "sum_{s in B} p_s Delta_s + C_comp",
        },
        {
            "metric": "decomposition_error",
            "value": decomposition_error,
            "description": "VSS_from_decomposition - VSS",
        },
        {
            "metric": "number_good_scenarios",
            "value": len(good_rows),
            "description": "|G| where G = {s: Delta_s <= 0}",
        },
        {
            "metric": "number_bad_scenarios",
            "value": len(bad_rows),
            "description": "|B| where B = {s: Delta_s > 0}",
        },
        {
            "metric": "minimal_bad_subset_size",
            "value": len(minimal_subset_rows),
            "description": "Smallest |K| found such that sum_{s in K} p_s Delta_s + C_comp > 0",
        },
        {
            "metric": "minimal_bad_subset_weighted_delta_sum",
            "value": running_weighted_delta,
            "description": "sum_{s in K} p_s Delta_s for selected K",
        },
        {
            "metric": "minimal_bad_subset_plus_C_comp",
            "value": selected_total_with_comp,
            "description": "sum_{s in K} p_s Delta_s + C_comp",
        },
    ]

    return {
        "Delta_0": delta_0,
        "C_comp": compensation_budget,
        "good_weighted_sum": good_weighted_sum,
        "bad_weighted_sum": bad_weighted_sum,
        "VSS_from_decomposition": vss_from_decomposition,
        "decomposition_error": decomposition_error,
        "regret_rows": regret_rows,
        "good_rows": good_rows,
        "bad_rows": bad_rows,
        "minimal_bad_subset_rows": minimal_subset_rows,
        "minimal_bad_subset_status": minimal_subset_status,
        "minimal_bad_subset_weighted_delta_sum": running_weighted_delta,
        "minimal_bad_subset_plus_C_comp": selected_total_with_comp,
        "regret_summary_rows": regret_summary_rows,
    }
...
def make_model_summary_rows(
    ...
    rows.extend(
        [
            {
                "quantity": "Delta_0",
                "value": regret["Delta_0"],
                "description": "First-stage difference c*x_EV - c*x_star",
            },
            {
                "quantity": "C_comp",
                "value": regret["C_comp"],
                "description": "Compensation budget Delta_0 + sum_{s in G} p_s Delta_s",
            },
            {
                "quantity": "minimal_bad_subset_size",
                "value": len(regret["minimal_bad_subset_rows"]),
                "description": "Smallest number of bad scenarios selected",
            },
            {
                "quantity": "minimal_bad_subset_plus_C_comp",
                "value": regret["minimal_bad_subset_plus_C_comp"],
                "description": "sum_{s in K} p_s Delta_s + C_comp",
            },
        ]
    )
...
def print_regret_decomposition(regret: Dict[str, Any], vss: float) -> None:
    print("\n" + "=" * 80)
    print("Scenario-wise regret decomposition")
    print("=" * 80)
    print(f"Delta_0 = c*x_EV - c*x_star = {regret['Delta_0']:.4f}")
    print(f"sum good p_s Delta_s       = {regret['good_weighted_sum']:.4f}")
    print(f"sum bad  p_s Delta_s       = {regret['bad_weighted_sum']:.4f}")
    print(f"C_comp                      = {regret['C_comp']:.4f}")
    print(f"VSS                         = {vss:.4f}")
    print(f"VSS from decomposition      = {regret['VSS_from_decomposition']:.4f}")
    print(f"decomposition error         = {regret['decomposition_error']:.8f}")

    if vss <= EPS:
        print("\nVSS <= 0, so the minimal bad scenario subset is not economically meaningful.")
    ...
    for row in regret["regret_rows"]:
        print(
            f"{row['scenario']:>3} "
            f"{row['demand']:>8.2f} "
            f"{row['Q_s_x_star']:>10.2f} "
            f"{row['Q_s_x_EV']:>10.2f} "
            f"{row['Delta_s']:>12.2f} "
            f"{row['p_s_Delta_s']:>12.2f} "
            f"{row['classification']:>8}"
        )

    good_ids = [row["scenario"] for row in regret["good_rows"]]
    bad_ids = [row["scenario"] for row in regret["bad_rows"]]
    min_ids = [row["scenario"] for row in regret["minimal_bad_subset_rows"]]
    print(f"\nGood scenarios G = {good_ids}")
    print(f"Bad scenarios  B = {bad_ids}")
    print(f"Minimal bad subset status = {regret['minimal_bad_subset_status']}")
    print(f"Minimal bad subset K = {min_ids}")
    print(
        "sum_{s in K} p_s Delta_s + C_comp = "
        f"{regret['minimal_bad_subset_plus_C_comp']:.4f}"
    )
...
def write_text_summary(
    ...
    add("Scenario-wise regret decomposition")
    add("-----------------------------------")
    add("Delta_s = Q_s(x_EV) - Q_s(x_star)")
    add("Delta_0 = c*x_EV - c*x_star")
    add(f"Delta_0 = {regret['Delta_0']:.4f}")
    add(f"sum_{{s in G}} p_s Delta_s = {regret['good_weighted_sum']:.4f}")
    add(f"sum_{{s in B}} p_s Delta_s = {regret['bad_weighted_sum']:.4f}")
    add(f"C_comp = Delta_0 + sum_{{s in G}} p_s Delta_s = {regret['C_comp']:.4f}")
    add(f"VSS from decomposition = {regret['VSS_from_decomposition']:.4f}")
    add(f"Decomposition error = {regret['decomposition_error']:.8f}")
    add("")
    add(
        f"{'s':>3} {'demand':>8} {'Q_star':>10} {'Q_EV':>10} "
        f"{'Delta_s':>12} {'p Delta_s':>12} {'class':>8}"
    )
    for row in regret["regret_rows"]:
        add(
            f"{row['scenario']:>3} "
    ...
    add("")
    add(f"Good scenarios G = {[row['scenario'] for row in regret['good_rows']]}")
    add(f"Bad scenarios  B = {[row['scenario'] for row in regret['bad_rows']]}")
    add("")

    add("Minimal bad scenario subset")
    add("---------------------------")
    add(f"Status = {regret['minimal_bad_subset_status']}")
    add(f"K = {[row['scenario'] for row in regret['minimal_bad_subset_rows']]}")
    add(
        "sum_{s in K} p_s Delta_s = "
        f"{regret['minimal_bad_subset_weighted_delta_sum']:.4f}"
    )
    add(
        "sum_{s in K} p_s Delta_s + C_comp = "
        f"{regret['minimal_bad_subset_plus_C_comp']:.4f}"
    )
    if regret["minimal_bad_subset_rows"]:
        add("")
        add(
            f"{'rank':>5} {'s':>3} {'demand':>8} {'Delta_s':>12} "
            f"{'p Delta_s':>12} {'cum p Delta':>14} {'cum + C_comp':>14}"
        )
        for row in regret["minimal_bad_subset_rows"]:
            add(
                f"{row['selection_rank']:>5} "
                f"{row['scenario']:>3} "
                f"{row['demand']:>8.2f} "
                f"{row['Delta_s']:>12.2f} "
                f"{row['p_s_Delta_s']:>12.2f} "
                f"{row['cumulative_p_s_Delta_s']:>14.2f} "
                f"{row['cumulative_plus_C_comp']:>14.2f}"
            )
    add("")
    ...
    add(
        ...
        "The reported x_star, Q_s(x_star), regrets, and minimal bad subset are computed "
        "from the value returned by the solver."
    )
...
def save_outputs(
    ...
    comparison_rows = [
        ...
        {"metric": "Delta_0", "value": regret["Delta_0"]},
        {"metric": "C_comp", "value": regret["C_comp"]},
        {"metric": "VSS_from_decomposition", "value": regret["VSS_from_decomposition"]},
        {"metric": "minimal_bad_subset_size", "value": len(regret["minimal_bad_subset_rows"])},
        {
            "metric": "minimal_bad_subset_plus_C_comp",
            "value": regret["minimal_bad_subset_plus_C_comp"],
        },
    ...
    write_csv(output_dir / "scenario_regrets.csv", regret["regret_rows"])
    write_csv(output_dir / "good_scenarios.csv", regret["good_rows"])
    write_csv(output_dir / "bad_scenarios.csv", regret["bad_rows"])
    write_csv(output_dir / "minimal_bad_subset.csv", regret["minimal_bad_subset_rows"])
    write_csv(output_dir / "regret_summary.csv", regret["regret_summary_rows"])
...
def run_tssp(solver_name: str, output_dir: Path, tee: bool = False) -> Dict[str, Any]:
    ...
    regret = compute_regret_decomposition(
        data=data,
    ...
    print_regret_decomposition(regret, VSS)
    ...
    print(f"Scenario regrets:        {(output_dir / 'scenario_regrets.csv').resolve()}")
    print(f"Good scenarios:          {(output_dir / 'good_scenarios.csv').resolve()}")
    print(f"Bad scenarios:           {(output_dir / 'bad_scenarios.csv').resolve()}")
    print(f"Minimal bad subset:      {(output_dir / 'minimal_bad_subset.csv').resolve()}")
    print(f"Regret summary:          {(output_dir / 'regret_summary.csv').resolve()}")
    ...
    return {
        ...
        "EEV": EEV,
        "VSS": VSS,
        "Delta_0": regret["Delta_0"],
        "C_comp": regret["C_comp"],
        "good_scenarios": [row["scenario"] for row in regret["good_rows"]],
        "bad_scenarios": [row["scenario"] for row in regret["bad_rows"]],
        "minimal_bad_subset": [row["scenario"] for row in regret["minimal_bad_subset_rows"]],
        "output_dir": output_dir,
    }
