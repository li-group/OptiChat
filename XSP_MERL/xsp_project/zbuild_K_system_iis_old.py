...
def recourse_objective_expr(model, first_stage_var_names, keep_constant=False):
    """
    Extract q_s^T y_s from the scenario objective by dropping terms
    involving first-stage variables.

    For this DCAP model, the true recourse objective should normally
    have no constant. A nonzero constant often means fixed first-stage
    objective terms leaked into the expression.
    """
    ...
    expr = 0
    constant = pyo.value(repn.constant or 0.0)

    if abs(constant) > 1e-9:
        print(f"WARNING: objective constant detected: {constant}")
    ...
    for var, coef in zip(repn.linear_vars, repn.linear_coefs):
        if var.name in first_stage_pyomo_names:
            first_stage_terms.append((var.name, float(coef)))
        else:
            expr += coef * var
            recourse_terms.append((var.name, float(coef)))
...
for scen in K_scenarios:
    ...
    # Extract q_s^T y_s before fixing x, so first-stage terms
    # do not become constants.
...
body_constant = pyo.value(body_repn.constant or 0.0)
...
print("LP effective RHS would be:", budget_rhs - body_constant)
print("sum_{s in K} p_s:", sum_p_K)
print("sum_p_K * c^T x_EV:", expected_first_stage_leak)

if abs(body_constant) > 1e-7:
    print("\nWARNING: K-budget body has a nonzero constant.")

if abs(body_constant - expected_first_stage_leak) <= 1e-6:
    print("WARNING: This constant appears to be leaked fixed first-stage cost.")


M.K_budget = pyo.Constraint(
    expr=K_budget_body <= budget_rhs
)
...
check_val = pyo.value(M_check.CheckObjective)

print("Stored sum_K p_s Q_s(x_EV):", sum_K_p_QEV)
print("Solved K recourse objective:", check_val)
print("Difference:", check_val - sum_K_p_QEV)

if abs(check_val - sum_K_p_QEV) > 1e-6:
    raise RuntimeError(
