...
#the total purchase level from vendors a, b, c, have to be less than 150000
def subset_purchase_rule(model):
    return sum(model.pl[v, s] for (v, s) in model.vs if v in ['a', 'b', 'c']) <= 150000
model.subset_purchase = Constraint(rule=subset_purchase_rule)
