...
model.pfd = Param(model.p, model.pf, initialize ={
    ...
    ('three', 'min-prod'):  700, ('three', 'max-prod'):  1500, ('three', 'prod-cost'): 38}, default = 0, mutable = True, doc='production facility data (table 12.1)')
...
model.dcd = Param(model.d, model.dcp, initialize ={
   ...
   }, default = 0, mutable = True, doc='distribution center data (table 12.3)') 
model.czd = Param(model.c, model.czp, initialize ={
   ...
   (3,'min-demand'): 2000, (3,'max-demand'):  1000, (3,'revenue'):  65,
   (4,'min-demand'): 1500, (4,'max-demand'):  2000, (4,'revenue'):  72,
   (5,'min-demand'): 1500, (5,'max-demand'):  3000, (5,'revenue'):  71
   }, default = 0, mutable = True, doc='customer zone data (table 12.5)') 

# def pc_init(model, p, m):
#     key_list = list(model.m)
#     m_idx = next((idx for idx, key in enumerate(key_list) if key == m)) + 1
#     return model.pfd[p,'prod-cost'] + floor(2**(m_idx-1)/2)
# model.pc = Param(model.p, model.m, initialize=pc_init)   
...
# def pco_init(model, p, m):
#     key_list = list(model.m)
#     m_idx = next((idx for idx, key in enumerate(key_list) if key == m)) + 1
#     if m_idx < len(model.m):
#         return model.pfd[p,'over-cost'] + m_idx - 1
#     else:
#         return model.pfd[p,'over-cost'] + m_idx
# model.pco = Param(model.p, model.m, initialize=pco_init) 
