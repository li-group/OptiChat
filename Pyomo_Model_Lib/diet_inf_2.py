...
model.maxbudget = Param(initialize=0.105, mutable = True)
...
def vegan_rule(model):
    return model.x['cannedmilk'] + model.x['cheese'] + model.x['liver'] + model.x['porkroast'] + model.x['salmon'] + model.x['lard'] == 0;
model.vegan = Constraint(rule=vegan_rule,doc='vegan diet')

def budget_rule(model):
    return model.cost <= model.maxbudget;
model.budget = Constraint(rule=budget_rule,doc='budget constraint')
