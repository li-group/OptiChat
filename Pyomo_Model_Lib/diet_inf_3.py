...
def fruit_rule(model):
    return sum(model.x[f] for f in model.f if f not in ['peaches','prunes']) == 0 ;
model.fruit = Constraint(rule=fruit_rule,doc='fruit only diet')

def budget_rule(model):
    return model.cost <= model.maxbudget;
model.budget = Constraint(rule=budget_rule,doc='budget limit')
