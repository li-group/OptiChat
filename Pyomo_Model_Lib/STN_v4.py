...
Kondili = {
    ...
    # states
    'STATES': {
        'Feed_A': {'capacity': 500, 'initial': 25, 'price': 0},
        'Feed_B': {'capacity': 500, 'initial': 25, 'price': 0},
        'Feed_C': {'capacity': 500, 'initial': 25, 'price': 0},
...
# store pamameters
model.rho = Param(rho.keys(), mutable=True, initialize=rho)
model.rho_ = Param(rho_.keys(), mutable=True, initialize=rho_)
model.C = Param(C.keys(), mutable=True, initialize=C)
model.Bmax = Param(Bmax.keys(), mutable=True, initialize=Bmax)
model.Bmin = Param(Bmin.keys(), mutable=True, initialize=Bmin)
model.Pi = Param(Pi.keys(), mutable=True, initialize=Pi)
...
model.sc = Constraint(STATES.keys(), TIME, rule=lambda model, s, t: model.S[s, t] <= model.C[s])
