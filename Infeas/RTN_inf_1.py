...
# with open("RTN_inf_1_data.json", "r") as file:
#     data = json.load(file)
...
model.Xmax = pe.Param(model.R, mutable=True, initialize = {r: net.nodes[r]["Xmax"] / 2 for r in model.R}, doc="Xmax (maximum resource inventory)")
model.Xmin = pe.Param(model.R, mutable=True, initialize = {r: net.nodes[r]["Xmin"] / 2 for r in model.R}, doc="Xmin (minimum resource inventory)")
