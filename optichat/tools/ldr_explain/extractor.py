...
class LDRExtractor:
                ...
                for f in fs:
                    if isinstance(f, (int, float)):
                        ...
                        if k in uids:
                            ...
                            coef *= float(f.value)
                    elif isinstance(f, (SumExpression, LinearExpression)):
                        if lin_term is None:
                            lin_term = f
                        else:
    ...
    def extract_primal_md(
        ...
        to classify variables as 'cont' | 'integer' | 'binary'.
        """
    ...
    def build_var_xi_map(
        ...
        *,
        ...
        for v in all_vs:
            ...
            ti = khop_collect(v, k, var_to_ti_direct)

            # Temporal: windowing by default; optionally k-hop if requested
            if khop_temporal:
                tt = khop_collect(v, k, var_to_tt_direct)
            else:
                tt = set()
                vtpos = v_tpos(vb)
